...
======================================================================================
更新时间：2022-12-05
当前脚本只适用于  “X86的alpine系统”  安装甜糖服务，若选错了按Ctrl+C即可结束安装
 
  请输入下列序号，进行相应操作
	
	1.一键部署docker甜糖服务（开启高质量通道，内存≥2G，硬盘≥128G，带宽≥100Mbps）
	2.一键部署docker甜糖服务（非高质量通道，硬盘≥32G）
	3.一键部署甜糖服务（二进制程序）
	4.硬盘分区并格式化（更换缓存盘后使用）
	5.清除甜糖缓存（不会删除绑定信息）
	6.删除甜糖容器（docker）
	7.更新甜糖容器（docker，高质量通道）
	8.更新甜糖容器（docker，非质量通道）
	9.删除甜糖服务（二进制程序）
	12.退出
